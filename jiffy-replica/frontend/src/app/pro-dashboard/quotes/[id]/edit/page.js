...
import { fetchQuoteById, updateQuote, sendQuote } from '@/store/slices/quotesSlice';
...
export default function EditQuotePage() {
  ...
  const { currentQuote, isLoading } = useSelector((state) => state.quotes);
  ...
  // Populate form from fetched quote
  useEffect(() => {
    if (currentQuote && !loaded) {
      setTitle(currentQuote.title || '');
      setDescription(currentQuote.description || '');
      setCustomerId(currentQuote.customer_id || '');
      setValidUntil(currentQuote.valid_until ? currentQuote.valid_until.split('T')[0] : '');
      setDiscountAmount(String(currentQuote.discount_amount || '0'));
      setNotes(currentQuote.notes || '');

      if (currentQuote.customer) {
        setSelectedCustomer({
          id: currentQuote.customer.id,
          full_name: currentQuote.customer.full_name,
          email: currentQuote.customer.email,
        });
      }

      const quoteItems = currentQuote.quote_items || [];
      if (quoteItems.length > 0) {
        ...
        setItems([{ description: '', quantity: '1', unit_price: '', unit: 'each' }]);
      }
      setLoaded(true);
    }
  }, [currentQuote, loaded]);
  ...
  const handleSubmit = async (sendImmediately = false) => {
    ...
    try {
      ...
      const result = await dispatch(updateQuote({ id, data: quoteData })).unwrap();
      ...
      router.push(`/pro-dashboard/quotes/${id}`);
    } catch (err) {
      toast.error(err || 'Failed to update quote');
    }
    setSaving(false);
  };
  ...
  if (currentQuote && !['draft', 'sent', 'viewed'].includes(currentQuote.status)) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white rounded-xl border shadow-sm p-8 text-center max-w-md">
          <p className="text-lg font-semibold text-gray-900 mb-2">Cannot Edit Quote</p>
          <p className="text-gray-500 mb-4">This quote has status &ldquo;{currentQuote.status}&rdquo; and can no longer be edited.</p>
          <Link href={`/pro-dashboard/quotes/${id}`} className="text-blue-600 hover:underline font-medium">
            View Quote
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
          <Link href={`/pro-dashboard/quotes/${id}`} className="flex items-center gap-2 text-gray-500 hover:text-gray-700 mb-4 text-sm">
            <ArrowLeft className="w-4 h-4" />
            Back to Quote
          </Link>
          <h1 className="text-2xl font-bold text-gray-900">Edit Quote</h1>
          {currentQuote?.quote_number && (
            <p className="text-gray-500 text-sm font-mono mt-1">{currentQuote.quote_number}</p>
          )}
        </div>
      </div>
