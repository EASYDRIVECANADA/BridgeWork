...
import {
  FileText, Receipt, Eye, Loader2, CheckCircle, XCircle,
  DollarSign, Clock, ChevronRight
} from 'lucide-react';
import { fetchQuotes, fetchInvoices } from '@/store/slices/quotesSlice';
...
export default function CustomerQuotesPage() {
  ...
  const { quotes, invoices, isLoading } = useSelector((state) => state.quotes);
  ...
  // Only show quotes that have been sent to this customer
  const visibleQuotes = (quotes || []).filter(q => ['sent', 'viewed', 'accepted', 'declined', 'expired', 'converted'].includes(q.status));
  const visibleInvoices = (invoices || []).filter(i => ['sent', 'paid', 'partially_paid', 'overdue'].includes(i.status));

  const pendingQuotes = visibleQuotes.filter(q => ['sent', 'viewed'].includes(q.status));
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
          ...
          <p className="text-gray-500 mt-1">View quotes and invoices from your service professionals</p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700 mt-0.5">
                  {pendingQuotes.length > 0 && `${pendingQuotes.length} quote(s) waiting for your response. `}
                  {pendingInvoices.length > 0 && `${pendingInvoices.length} invoice(s) awaiting payment.`}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex gap-1 bg-gray-100 rounded-lg p-1 mb-6 w-fit">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition ${
              activeTab === 'invoices' ? 'bg-white shadow text-green-600' : 'text-gray-600 hover:text-gray-900'
            }`}
          >
        ...
        {isLoading ? (
          ...
          visibleQuotes.length > 0 ? (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 text-sm text-gray-500">
                        <span>From: {quote.pro_profiles?.business_name || quote.pro_profiles?.profiles?.full_name || 'Pro'}</span>
            ...
            <div className="text-center py-20 bg-white rounded-xl border">
              ...
              <p className="text-gray-500">Quotes from your service professionals will appear here</p>
            </div>
          )
        ) : (
