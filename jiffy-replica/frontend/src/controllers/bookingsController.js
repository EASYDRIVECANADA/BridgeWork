...
exports.createBooking = async (req, res) => {
    try {
        ...
        // - 'awaiting_quotes' for Free Quote services (pros submit quotes, admin selects)
        // - 'pending' for regular services (goes directly to pros for acceptance)
        ...
        // Notify nearby pros for both regular and Free Quote bookings
        // - Regular bookings: pros can accept directly
        // - Free Quote bookings: pros can submit their quotations
        let nearbyPros = [];
        ...
        for (const pro of nearbyPros.slice(0, 10)) {
            if (isFreeQuote) {
                // Notify pros to submit quotations
                await createNotification(pro.user_id, {
                    type: 'booking',
                    title: 'Quote Request Available',
                    message: `New quote request for ${service.name} in ${city}, ${state}. Submit your quotation!`,
                    link: `/pro-dashboard/quote-requests/${booking.id}`,
                    data: { booking_id: booking.id, type: 'quote_request' }
                });
            } else {
        ...
        logger.info('Booking created', { 
            ...
            isFreeQuote,
            status: bookingStatus,
            prosNotified: nearbyPros.length
        });

        res.status(201).json({
            success: true,
            message: isFreeQuote 
                ? 'Quote request submitted! Service professionals will provide quotes shortly.'
...
/**
 * Pro: Get all quote requests available for bidding
...
exports.getQuoteRequestsForPro = async (req, res) => {
    try {
        ...
        // Get bookings awaiting quotes - try with booking_quotations first
        let bookings = [];
        let queryError = null;

        // First try with booking_quotations join (if table exists)
        const { data: bookingsWithQuotes, error: error1 } = await supabaseAdmin
            .from('bookings')
            .select(`
                *,
                services (id, name, category_id, image_url),
                profiles!bookings_user_id_fkey (id, full_name, avatar_url),
                booking_quotations (id, pro_id, status)
            `)
            .eq('status', 'awaiting_quotes')
            .order('created_at', { ascending: false });

        if (error1) {
            // If booking_quotations table doesn't exist, try without it
            logger.info('booking_quotations table may not exist, trying without join');
            const { data: bookingsSimple, error: error2 } = await supabaseAdmin
                .from('bookings')
                .select(`
                    *,
                    services (id, name, category_id, image_url),
            ...
            if (error2) {
                // Status might not exist either - return empty array
                logger.info('awaiting_quotes status may not exist yet');
                bookings = [];
            } else {
                bookings = bookingsSimple || [];
            }
        } else {
            bookings = bookingsWithQuotes || [];
        }

        // Filter out bookings where this pro already submitted a quote
        const availableBookings = bookings.map(booking => {
            const myQuote = booking.booking_quotations?.find(q => q.pro_id === proProfile.id);
            return {
                ...booking,
                has_submitted_quote: !!myQuote,
                my_quote_status: myQuote?.status || null,
                total_quotes: booking.booking_quotations?.length || 0
            };
        });
    ...
    try {
        ...
        for (const admin of admins || []) {
            await createNotification(admin.id, {
                ...
                message: `${proProfile.business_name || 'A pro'} submitted a quote of $${quoted_price} for ${booking.service_name}`,
        ...
        res.status(201).json({
            success: true,
            message: 'Quotation submitted successfully! Admin will review and notify you if selected.',
            data: { quotation }
        });
    } catch (error) {
